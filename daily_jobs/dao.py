...
class FormParser(object):
    ...
    def insert_parser(self, form_type, code):
        today_str = datetime.today().strftime('%Y-%m-%d')
        sql = f"INSERT INTO form_parser(form_type, code, date_created, date_modified) VALUES('{form_type}', {code[1:]}, " \
            f"'{today_str}', '{today_str}') ON DUPLICATE KEY UPDATE code={code[1:]}, date_modified='{today_str}';"
